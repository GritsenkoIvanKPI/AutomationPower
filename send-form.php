<?php
/**
 * Endpoint для форм «Отримати прорахунок» — приймає заявку з будь-якої сторінки
 * сайту й пересилає її в Telegram.
 *
...
$type    = field($input, 'type', 20);
$comment = field($input, 'comment', 2000);
...
// Український номер — 9 цифр після коду, але приймаємо і міжнародні формати.
$digits = preg_replace('/\D+/', '', $phone) ?? '';
if (strlen($digits) < 7) {
    $errors['phone'] = 'invalid';
}

// На цій формі пошта обовʼязкова — заявки B2B ведуться листуванням.
...
if ($limit > 0) {
    ...
    $bucket = sys_get_temp_dir() . '/automatonpower-form-' . hash('sha256', $ip) . '.txt';
    $now    = time();
    $hits   = [];
...
// Назви напрямів беремо зі словника, а не з того, що надіслав браузер, — інакше
// сюди можна було б підставити довільний текст.
$TYPES = [
    't21700' => 'Елементи 21700',
    'hd'     => 'Акумулятори високої щільності',
    'ev'     => 'Пакети для електротранспорту',
    'other'  => 'Інше / потрібна консультація',
];
...
$lines   = [];
$lines[] = '<b>⚡ Нова заявка з сайту Automaton Power</b>';
$lines[] = '';
$lines[] = '<b>Імʼя:</b> ' . tg($name);
$lines[] = '<b>Телефон:</b> ' . tg($phone);
$lines[] = '<b>Email:</b> ' . tg($email);
$lines[] = '<b>Тип виробу:</b> ' . tg($typeLabel);

if ($comment !== '') {
    $lines[] = '';
    $lines[] = '<b>Коментар / ТЗ:</b>';
    $lines[] = tg($comment);
}
...
if ($notify !== '' && filter_var($notify, FILTER_VALIDATE_EMAIL)) {
    $body = "Імʼя: $name\nТелефон: $phone\nEmail: $email\nТип виробу: $typeLabel\n"
          . "Коментар: $comment\nСторінка: $page\n";
    @mail(
        $notify,
        'Нова заявка з сайту Automaton Power',
        $body,
